	...
	<view class="container">
		...
		<view class="bg-top bg-cyan">
			...
			<view class="padding-lr flex margin-top-sm" style="justify-content: space-between;">
				<button class="bg-blue bg-green margin-top-sm lg" style="width: 46%;"  @click="receiptApprove(5)">拒绝</button>
				<button class="cu-btn bg-green margin-top-sm lg" style="width: 46%;"  @click="receiptApprove(3)">审核通过</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			receiptApprove(state){
				var that = this;
					let opts = {
						url: 'warehouse/app/outReceiptApprove/'+that.receiptId+'?state='+state,
						method:'get'
					};
