							...
							<view class="">
								<u-number-box v-model="item.goodsNumber" button-size="36" color="#ffffff"
									inputWidth="60" min="0" bgColor="#2979ff" iconStyle="color: #fff"
									 :min="0" :max="item. stockNumberTmp"
									@change="valueChange(item, item.goodsNumber, $event)"
									></u-number-box>
							</view>
						</view>
	...
	export default {
	  filters:{
		...
		onLoad(options) {
			...
			this.warehouseIds = params.warehouseIds;
			var that = this;
		...
		methods: {
			...
			_getGoodsList() {
				let opts = {
					url: 'warehouse/app/stock?warehouseIds='+this.warehouseIds,
					method: 'get',
				};
				...
				request.httpTokenRequest(opts).then(res => {
					...
					if (res.code == 200) {
						...
						for (var k = 0, length = res.rows.length; k < length; k++) {
							...
							newgoods.stockNumberTmp=newgoods.stockNumber;
							newgoods.goodsPicture = baseUrl + newgoods.goodsPicture.substring(1);
							newgoods.createTime = newgoods.createTime.substring(0,10)+' '+newgoods.createTime.substring(12,19);
							newgoods.updateTime = newgoods.updateTime.substring(0,10)+' '+newgoods.updateTime.substring(12,19);
						}
			...
			// },
			    ...
			    valueChange(row, prop, event) {
				  let newda = row.stockNumberTmp - event.value;
				  row.stockNumber = newda;
			      // 这里可以执行其他逻辑，比如更新服务器数据等
			    },
			truncateGoods() {
