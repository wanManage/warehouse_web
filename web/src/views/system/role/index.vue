<template>
  <div class="app-container">
        ...
        <template slot-scope="scope" v-if="scope.row.roleId !== 1">
          ...
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['system:role:edit']">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item command="handleProcess" icon="el-icon-user" v-hasPermi="['system:role:edit']">流程分配</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <!-- 流程分配对话框 -->
    <el-dialog title="流程" :close-on-press-escape="false" :close-on-click-modal="false" :visible.sync="showProcessDialog">
      <el-tree ref="processTree" :data="processData" node-key="procCode" :default-checked-keys="processCheckedKeys" default-expand-all :props="procProps" show-checkbox>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="saveRoleProcess">保 存</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { processTree, getProcessCodeByRoleId, updateRoleProcessCode } from "@/api/process/ehrProcess";
export default {
  name: "Role",
  ...
  methods: {
    ...
    initProcess() {
      ...
      processTree().then((res) => {
        this.processData = res.data;
      });
      getProcessCodeByRoleId(this.procRoleId).then((res) => {
        this.processCheckedKeys = res.data;
      });
    },
    // 保存角色流程配置信息
    saveRoleProcess() {
      this.$confirm("是否确认修改角色流程配置？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        let checkedNodes = this.$refs.processTree.getCheckedNodes();
        updateRoleProcessCode({
          roleId: this.procRoleId,
          processCodeList: checkedNodes.filter((item) => item.procCode).map((item) => item.procCode),
        }).then((res) => {
          this.$notify({
            title: "成功",
            message: "修改成功",
