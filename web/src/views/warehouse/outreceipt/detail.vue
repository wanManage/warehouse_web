  ...
  <div class="app-container">
    <el-card>
      <div slot="header" class="clearfix">
        ...
        <el-button style="float: right; margin-left: 10px" icon="el-icon-s-check" type="danger" v-if="form.state == 2" v-hasPermi="['warehouse:outreceipt:approve']" @click="rejectInReceipt">拒绝</el-button>
        <el-button style="float: right; margin-left: 10px" icon="el-icon-s-check" type="primary" v-if="form.state == 2" v-hasPermi="['warehouse:outreceipt:approve']" @click="approveOutReceipt">审核</el-button>
      </div>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item v-if="form.outWarehouseType == 99 || form.outWarehouseType == 65" label="接收仓库" prop="receiveId">
...
import { getOutreceipt, outReceiptPdf,updateOutreceipt, approveOutreceipt } from "@/api/warehouse/outreceipt";
...
export default {
  ...
  methods: {
    ...
    printOutReceipt() {
     ...
     rejectInReceipt(){
      this.$modal
        .confirm("是否确认拒绝该请求？提交之后将不可撤回！")
        .then((t) => {
          this.form.state=5
          approveOutreceipt(this.form).then((response) => {
             this.close();
          });
        })
        .catch();
