  ...
  <div class="app-container" style="height: 100%">
    <el-card>
      <div slot="header" class="clearfix">
        <span>要货单明细</span>
        <el-button style="float: right; margin-left: 10px" type="primary" @click="submitForm(1)">生成出库单</el-button>
        <el-button style="float: right" type="warning" @click="submitForm(0)">拒绝</el-button>
      </div>
      <el-form ref="applyForm" :model="applyForm" :rules="rules" label-width="100px">
        ...
        <el-form-item label="出货仓库" prop="outWarehouseId">
          <el-select v-model="applyForm.outWarehouseId" placeholder="请选择出货仓库" @change="warehouseChange" clearable>
    ...
    <el-card style="margin-top: 20px">
      ...
      <el-table :data="applyGoodsList">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-circle-plus" @click="handleAdd(scope.row)">添加到出库单</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-dialog :title="title" :visible.sync="open" width="1124px" :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true">
      <el-table v-loading="loading" :data="goodsList" @selection-change="handleSelectionChange" @row-dblclick="chooseGoods" row-class-name="not-select">
      ...
      <el-table :data="selectGoodsList" @selection-change="handleGoodsSelectionChange">
        <el-table-column label="物品类别" align="center" prop="categoryName" />
        <el-table-column label="物品编号" align="center" prop="goodsCode" />
        <el-table-column label="物品名称" align="center" prop="goodsName" />
        <el-table-column label="规格型号" align="center" prop="specificationType">
        ...
        <el-table-column label="操作" align="center" prop="colorType">
          <template slot-scope="scope">
            <el-button type="text" @click="removeGoods(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { getApply, updateApply, agreeApply, rejectApply, delApply } from "@/api/warehouse/apply";
export default {
  name: "ApplyEdit",
  ...
  methods: {
        ...
        if (valid) {
          this.$modal
            ...
            .then((t) => {
              ...
              if (type == 1) {
                agreeApply(this.applyForm).then((response) => {
                  this.$modal.msgSuccess("生成出库单成功！请进行后续出库操作！");
                  this.close();
                });
              } else {
                rejectApply(this.applyForm).then((response) => {
                  this.$modal.msgSuccess("退回申请单成功！请进行后续出库操作！");
                  this.close();
                });
              }
            })
    ...
    handleSelectionChange(rows) {
      this.selectedRows = rows; // 更新选中的数据
    },
    // 弹窗事件
