  ...
  <div class="app-container">
    <el-card>
      <div slot="header" class="clearfix">
        <span>要货单明细</span>
        <el-button style="float: right; margin-left: 10px" type="primary" @click="submitForm(1)">提交</el-button>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="要货仓库" prop="applyWarehouseId">
          <el-select v-model="form.applyWarehouseId" filterable placeholder="请选择" @change="warehouseChange">
            <el-option v-for="item in outWarehouseList" :key="item.warehouseId" :label="item.warehouseName" :value="item.warehouseId"> 
        ...
        <el-form-item label="调入仓库" prop="applyInWarehouseId">
          <el-select v-model="form.applyInWarehouseId" filterable placeholder="请选择">
        ...
        <el-form-item label="要货类别" prop="applyType">
          <el-radio-group v-model="form.applyType">
            <el-radio-button v-for="dict in dict.type.apply_type" :key="dict.value" :label="dict.value" :value="dict.value">{{ dict.label }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
...
import { applyCode} from "@/api/warehouse/warehousecode";
import { getApply, addApply, updateApply } from "@/api/warehouse/apply";
import { deepClone } from "@/utils/index";
export default {
  name: "TransferView",
  dicts: ["goods_specification", "goods_color", "goods_unit", "apply_type", "sys_yes_no"],
  components: {},
  data() {
    return {
      // 表单参数
      ...
      // 表单校验
      rules: {
        applyWarehouseId: [{ required: true, message: "出仓仓库不能为空", trigger: "blur" }],
        applyInWarehouseId: [{ required: true, message: "入仓仓库不能为空", trigger: "blur" }],
  ...
  methods: {
    init() {
      listWarehouse().then((res) => {
        ...
        for (var k = 0, length = res.rows.length; k < length; k++) {
            ...
            if(newgoods.deptId == this.$store.state.user.dept.deptId){
              this.inWarehouseList.push(newgoods)
            }else{
              this.outWarehouseList.push(newgoods)
            }
        }
        console.log(user)
      });
    ...
    doSubmitForm() {
      ...
      this.form.applyInWarehouseName = this.inWarehouseList.filter((t) => t.warehouseId == this.form.applyInWarehouseId)[0].warehouseName;
      this.form.warehouseId = this.form.applyInWarehouseId
    ...
    checkGoodsList() {
      ...
      if (this.form.applyWarehouseId == this.form.applyInWarehouseId) {
        this.$message({
          type: "error",
          message: "调出和调入仓库不能一样",
        });
    ...
    getGoodsList() {
      this.loading = true;
      this.queryParams.warehouseId = this.form.applyWarehouseId;
