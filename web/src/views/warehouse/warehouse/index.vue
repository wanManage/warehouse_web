  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item v-if="form.warehouseType == 2" label="归属部门" prop="form.deptId">
          <treeselect v-model="form.deptId" :options="departments" :show-count="true" placeholder="请选择归属部门" />
        </el-form-item>

    <!--    <el-form-item v-if="form.warehouseType == 2" label="负责人" prop="headId" @click="getuUserList">
          <el-select v-model="form.headId" placeholder="请选择负责人" filterable>
            <el-option v-for="head in heads" :key="head.value" :label="head.label" :value="head.value"></el-option>
          </el-select>
        </el-form-item -->
...
export default {
  ...
  methods: {
    /** 查询部门下拉树结构 */
    getDeptTree() {
      if (checkPermi(['warehouse:warehouse:add','warehouse:warehouse:edit'])){
    ...
    getuUserList() {
      if (this.deptId != 0 ){
        this.userQueryParams.deptId = this.deptId
        listUser(this.userQueryParams).then((response) => {
          this.heads = response.rows.map(item => {
