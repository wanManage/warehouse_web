  ...
  <div class="app-container">
    ...
    <el-card>
      ...
      <el-table :data="receiptGoodsList">
        ...
        <el-table-column label="实际库存" align="center" prop="goodsNewNumber" width="140">
          <template slot-scope="scope">
            <el-input-number :precision="0" size="small" v-model="scope.row.goodsNewNumber"></el-input-number>
          </template>
        </el-table-column>
        <el-table-column label="说明" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="1124px" :close-on-click-modal="false" :close-on-press-escape="false" :destroy-on-close="true">
      <el-row :gutter="20">
        <el-col class="border-left" :span="24" :xs="24">
          ...
          <el-table v-loading="loading" :data="goodsList" @selection-change="handleGoodsSelectionChange" row-class-name="not-select">
...
import { checkCode } from "@/api/warehouse/warehousecode";
...
import { updateCheckReceipt, addCheckReceipt, getCheckReceipt } from "@/api/warehouse/check";
export default {
  name: "CheckEdit",
  components: {},
  ...
  methods: {
    ...
    doSubmitForm() {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.checkReceiptId != null) {
            updateCheckReceipt(this.form).then((response) => {
              this.$modal.msgSuccess("修改成功");
              this.close();
            });
          } else {
            addCheckReceipt(this.form).then((response) => {
    ...
    confirmChoose() {
      ...
      for (let i = 0; i < this.selectGoodsList.length; i++) {
        ...
        element.goodsOldNumber = element.stockNumber;
        element.goodsNewNumber = element.stockNumber;
